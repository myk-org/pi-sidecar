...
> **Note:** `agent_dir` must be an absolute path pointing to an existing directory (loopback). In `DEV_MODE`, it must still be a non-empty string (empty/whitespace → HTTP 400); a valid value is then discarded. On non-loopback binds without `DEV_MODE`, including `agent_dir` returns HTTP 400.
