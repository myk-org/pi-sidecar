...
2. **PATH filtering** — Applied in `startSidecar()` in `src/index.ts`. The sidecar's `node_modules/.bin/` contains a local `pi` binary from its `@earendil-works/pi-coding-agent` dependency, which may be a different version than the globally installed `pi`. If the local version is older, the spawned subprocess will fail with extension loading errors (e.g., missing APIs). The fix derives the sidecar's install root from `import.meta.url` (not `process.cwd()`) and conditionally strips its exact `node_modules/.bin` directory from PATH. The strip is guarded by checking whether a `pi` binary exists as a file in any of the remaining PATH directories — if `pi` would not be reachable after removal, the local entry is kept to avoid breaking subagent invocations entirely (logged as `PATH_FILTER_SKIPPED`).
